...
RegularExpressionParser::ParseTree RegularExpressionParser::Parse(string expression)
{
	...
	bool success = parser->errors->count == 0 && result.size() == 1;
	...
	while (!q.empty())
	{
		...
		for (const auto& it : item->Out)
		{
			if (em.find(it) == em.end())
			{
				...
				dot.AddEdge("V" + to_string((int)item), "V" + to_string((int)(it->Vf)), it->C );
			}
		...
		for (const auto& it : item->In)
		{
			if (em.find(it) == em.end())
			{
				...
				dot.AddEdge("V" + to_string((int)(it->Vs)), "V" + to_string((int)item), it->C);
			}
		}
	}
...
GraphicFiniteAuto* ParseTreeToFAConverter::GetFA() const
{
	return fa;
}
